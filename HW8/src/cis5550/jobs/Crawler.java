            ...
            try {
                ...
                while (rdd.count() != 0) {
                    ...
                    rdd = (FlameRDDImpl) rdd.flatMap(urls -> {
                        ...
                        if (visitrecord != null) {
                            if (visitrecord.get("respond").equals("200")) {
                                return results;
                            }
                        }
                        if (hostrecord == null) {
                        ...
                        try {
                            ...
                            System.out.println("time:"+curtimerow.get("delay")+"-"+timeValue);
...
//                            headconnection.
                            ...
                            if (headResponseCode == 200) {
                                ...
                                if (responseCode == 200) {
                                    ...
                                    r.put("url", urls);
                                    r.put("respond", "" + responseCode);
                                    r.put("page", "");
                                    context.getKVS().putRow("pt-crawl", r);
                                    System.out.print("add" + urls + "to pt");
                                    // context.output(urls);
                                    // context.output(content.toString());
                                    results = extracturl(content.toString());
                                ...
                                Row r = new Row(Hasher.hash(urls));
                                r.put("url", urls);
                                r.put("respond", "" + headResponseCode);
                                context.getKVS().putRow("pt-crawl", r);
                                String redirectUrl = headconnection.getHeaderField("Location");
                                results.add(redirectUrl);
                                System.out.print("redirect" + urls + "to" + headResponseCode + redirectUrl);
                                return results;
                            } else {
                                System.out.print("error" + urls + "to" + headResponseCode);
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
